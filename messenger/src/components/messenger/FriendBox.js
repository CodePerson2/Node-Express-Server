...
const FriendBox = ({ getMessages, darkMode, friend, onToggle }) => {
  ...
  const openChat = () => {
    onToggle(friend.groupID);
    getMessages(friend.groupID);
  }
  return (
    <div
      ...
      onClick={openChat}
    >
      <div className={`App-SideBar-Boxes-Box-name ${darkMode ? "text1-D" : "text1-L"}`}>{friend.userName}</div>
      <div className="App-SideBar-Boxes-Box-Cont">
        ...
        <div className={`App-SideBar-Boxes-Box-Cont-date ${darkMode ? "text2-D" : "text2-L"}`}>&#8226; {friend.messDate}</div>
      </div>
    </div>
  );
};
